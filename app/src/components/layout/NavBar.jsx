...
const TABS = [
  ...
  { to: '/reports',  label: 'Reports', svg: ClipboardIcon, end: false, idleAnim: 'animate-chart-float', requires: 'core' },
...
const tabClass = (isActive, locked) =>
  clsx('flex-1 flex flex-col items-center gap-1 py-2 text-xs transition-colors',
    locked ? 'text-slate-600' : isActive ? 'text-primary' : 'text-white');

function LockBadge() {
  return (
    <span className="absolute -top-0.5 -right-0.5 flex items-center justify-center w-3.5 h-3.5 rounded-full bg-slate-900 border border-slate-700">
      <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor"
        strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-slate-500">
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-30 bg-slate-900/80 backdrop-blur-md border-t border-slate-700/60 pb-safe">
      <div className="max-w-2xl md:max-w-3xl lg:max-w-5xl xl:max-w-6xl mx-auto relative flex items-center">
        {/* Sliding pill indicator — only for unlocked tabs */}
        {activeIdx >= 0 && !TABS[activeIdx]?.requires && (
          <div
        ...
        {TABS.map((tab) => {
          ...
          const locked = tab.requires ? !has(tab.requires) : false;
          return (
            <NavLink
              key={tab.to}
              to={tab.to}
              end={tab.end}
              className={({ isActive }) => tabClass(isActive, locked)}
            >
              {({ isActive }) => (
                <>
