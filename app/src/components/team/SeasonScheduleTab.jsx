...
export function SeasonScheduleTab({ seasonId }) {
  ...
  const data = useLiveQuery(async () => {
    ...
    const rawMatches = (await db.matches.where('season_id').equals(seasonId).toArray())
  ...
  const seasonStats = useLiveQuery(async () => {
    ...
    const season = await db.seasons.get(seasonId);
    if (!season) return null;
    const completedMatches = await db.matches.where('season_id').equals(seasonId)
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (newRounds.length === 0) { completed.forEach(m => syncedMatchIds.current.add(m.id)); return; }
    ...
    try {
      ...
      if (editMatchId) {
        ...
        await db.transaction('rw', [db.matches, db.seasons], async () => {
          ...
          await db.seasons.update(seasonId, { peak_match_count: perSeasonPeak + 1 });
        });
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
            ...
            <div className="grid grid-cols-7 gap-2">
              {STATS.map(({ label, leaderKey, navKey, fmt }) => {
                ...
                return (
                  <button
                    key={leaderKey}
                    onClick={() => leader?.id && navigate(`/teams/${seasonStats.teamId}/players/${leader.id}?season=${seasonId}&stat=${navKey}`)}
                    disabled={!leader?.id}
            ...
            <div className="grid grid-cols-7 gap-2">
              {STATS.map(({ label, ttKey, fmt, noAvg }) => {
                ...
                return (
                  <button
                    key={ttKey}
                    onClick={() => navigate(`/seasons/${seasonId}/team?stat=${ttKey}`)}
            ...
            {(seasonStats.team.hit_pct != null || seasonStats.team.si_pct != null || seasonStats.team.ace_pct != null) && (
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <button
                    key={label}
                    onClick={() => navigate(`/seasons/${seasonId}/team?stat=${stat}`)}
          ...
          <section>
            ...
            <div className="space-y-3">
              {groups.map(([name, tMatches]) => {
                ...
                const tw = completed.filter(m => (m.our_sets_won ?? 0) > (m.opp_sets_won ?? 0)).length;
                const tl = completed.filter(m => (m.our_sets_won ?? 0) < (m.opp_sets_won ?? 0)).length;
                return (
                  <div key={name} className="bg-surface rounded-xl px-4 py-3">
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-semibold text-white text-sm">{name}</span>
                      {completed.length > 0 && (
                        <span className="text-sm font-mono font-bold">{tw}–{tl}</span>
                      )}
                    </div>
      ...
      <section>
        ...
        {matches.length === 0 ? (
          <div className="bg-surface rounded-xl px-5 py-6 text-center space-y-3">
            <div className="text-3xl">🏐</div>
            <div className="font-semibold text-white">No matches yet</div>
            <p className="text-sm text-slate-400">Schedule upcoming games ahead of time, or jump straight into recording a live match.</p>
            <div className="flex gap-3 justify-center pt-1">
              <Button variant="secondary" onClick={() => setSchedOpen(true)}>+ Schedule Game</Button>
              <Button onClick={() => navigate(`/matches/new?season=${seasonId}`)}>Start Match Now</Button>
            </div>
          </div>
        ) : (
          <div className="space-y-2">
            {matches.map(match => {
              if (match.status === MATCH_STATUS.SCHEDULED) {
                return (
                  <SwipeableMatchCard key={match.id} onDeleteConfirm={() => setConfirmDelete(match)}>
                    <div className="w-full bg-surface rounded-xl px-4 py-3 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => navigate(`/matches/new?season=${seasonId}&match=${match.id}`)}
      ...
      {confirmEndSeason && (
        <ConfirmDialog
          ...
          danger
          onConfirm={async () => {
            await db.seasons.update(seasonId, { status: 'ended' });
            await applyInferredSeasonFinish(seasonId, season.team_id, season.year);
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="bg-bg w-full max-w-md rounded-2xl p-6 space-y-4 shadow-2xl">
              ...
              <div className="flex gap-3 pt-2">
                <Button variant="secondary" className="flex-1" onClick={resetSchedForm}>Cancel</Button>
                <Button
                  className="flex-1"
