...
export const STAT_GLOSSARY = {
  ...
  // ── VER ───────────────────────────────────────────────────────────────────
  ver_raw:      { abbr: 'VER',      full: 'Raw Volleyball Efficiency Rating', def: 'Same formula as wVER, but WITHOUT the position multiplier applied. This is a player\'s pure output — everything they contributed, unadjusted for how many stat categories their position gives them access to.\n\nNot comparable across positions on its own: a Libero\'s raw ceiling is naturally far lower than an OH\'s, since a Libero can only ever draw from digs and passing, while an OH draws from attacking, serving, passing, digging, and blocking. A great Libero season and a great OH season can look very different in raw terms despite being equally valuable to the team — that\'s exactly what the position multiplier (see wVER) exists to correct for.\n\nUse VER to see a player\'s pure per-set production; use wVER to compare that production fairly across different positions.' },
  ver:          { abbr: 'wVER',     full: 'Position-Weighted Volleyball Efficiency Rating', def: 'Position-adjusted composite efficiency rating per set — VER (see above) multiplied by a position multiplier, so players in different roles can be compared on one shared scale.\n\nFormula:\nwVER = Multiplier × { (1 ÷ SP) × [\n  +4.00 × K\n  +4.00 × ACE\n  +5.00 × BS\n  +2.50 × BA\n  +1.00 × AST\n  +2.00 × (DIG + FREE)   ← freeball digs count equally\n  +(P1 + 2·P2 + 3·P3 − 2·REC)   ← APR component\n  −3.00 × AE\n  −3.00 × SE\n  −3.00 × BE\n  −3.00 × BHE\n  −3.00 × DROP\n  −3.00 × L\n  −3.00 × NET\n] + 13.3 × HIT% } ÷ 4\n\nBE (Block Errors) is included at the same −3.00 weight as every other error — a blocking mistake now costs a player just like an attack or service error does.\n\nHIT% Component: (K − AE) ÷ TA, added at ×13.3, outside the per-set division (it\'s already a rate, so it isn\'t diluted by playing time). A realistic hit% swing (.100 to .400) is worth about one kill\'s (4.00) worth of VER. Zero when the player has no attack attempts.\n\nThe whole formula is then divided by 4 — this is purely to keep the final numbers smaller and easier to read; it doesn\'t change how any stat compares to any other.\n\nAPR Component: (P1 + 2·P2 + 3·P3 − 2·REC)\n  Scores each pass relative to a neutral APR of 2.0:\n  P3 = +1 per pass (perfect)\n  P2 =  0 per pass (neutral)\n  P1 = −1 per pass (out of system)\n  P0 = −2 per pass (ace against)\n  Zero when the player has no pass attempts.\n\nPosition Multipliers (calibrated against real season data — each position\'s best real player was tested against their actual raw stats, and the multiplier solved so that season landed in its deserved tier; no longer round-number guesses, and OH is no longer a fixed "1.00 baseline"):\n  OH  2.70\n  OPP 2.70\n  RS  2.70\n  MB  4.75 (fewer stat categories available — rarely passes or digs)\n  S   4.00 (assists are weighted lower than kills per-action, needs the boost to compare fairly)\n  DS  5.00 (no offensive stats at all — must build VER entirely from digs and passing)\n  L   4.75 (no offensive stats at all — must build VER entirely from digs and passing)\n\nGrades (all positions — posMult already normalizes wVER cross-position):\n  ELITE   20.00 +\n  GREAT   13.00 – 19.99\n  GOOD    9.00 – 12.99\n  AVG     5.00 – 8.99\n  LOW     2.00 – 4.99\n  BENCH   0.01 – 1.99\n  NEG      < 0\n\nHigher is better. A wVER of 0 means the player broke even.' },
  ...
  lift:         { abbr: 'L',        full: 'Lifts',                      def: 'Contact where the ball is visibly lifted or carried. Penalized at −0.75 per set in VER.' },
  dbl:          { abbr: 'DBL',      full: 'Doubles',                    def: 'Illegal double-contact error. Penalized at −0.75 per set in VER, same as the other error types.' },
  net:          { abbr: 'NET',      full: 'Net Violations',             def: 'Contacts where the player touches the net during play. Penalized at −0.75 per set in VER.' },
  ...
  fbe:          { abbr: 'DROP',     full: 'Drop Error',                 def: 'An error where the ball hit the floor and the player was responsible. Penalized at −0.75 per set in VER.' },
  ...
  bcr:         { abbr: 'BCR',     full: 'Ball Control Rater',         def: 'Measures how effective a defender is at keeping every offensive option alive off their serve-receive passes, digs, and freeball digs. Passed on by a Division I women\'s head coach.\n\nFormula:\nBCR = [(P3 × 1.45) + (Perfect FREE × 0.16) + (Perfect DIG × 0.03)] ÷ [(REC × 1.45) + (FREE × 0.16) + (DIG × 0.03)]\n\n  P3 — perfect (3-rated) serve-receive passes\n  Perfect FREE — freeball digs rated 3\n  Perfect DIG — digs rated 3\n  REC — total serve-receive attempts\n  FREE — total freeball digs (rated or not)\n  DIG — total digs (rated or not)\n\nEach category is weighted by difficulty (serve receive hardest at 1.45, freeballs at 0.16, digs at 0.03) and every dig/freeball counts toward the total whether or not it was tagged with a rating — an untagged dig still counts as an attempt, it just can\'t count as "perfect."\n\nScale: 0–1, not scaled ×100. Higher is better.' },
